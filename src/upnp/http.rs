...
pub(crate) async fn trace_request(request: Request, next: axum::middleware::Next) -> Response {
    ...
    if uri.path().starts_with("/media/") || uri.path().starts_with("/art/") {
        trace_renderer(&peer, &method, uri.path(), status, &asked);
    }
    response
}
...
fn trace_renderer(peer: &str, method: &Method, path: &str, status: u16, headers: &HeaderMap) {
    let read = |name: &str| {
        headers
            .get(name)
            .and_then(|value| value.to_str().ok())
